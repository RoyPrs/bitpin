# -*- coding: utf-8 -*-
#
# todo/common/tests/record_creation.py
#
...
class RecordCreation:
    ...
    def _create_category(self, project, name, parent=None, user=None, **kwargs):
        """
    ...
    def _create_location_set_name(
        self, project, name=LOCATION_SET_NAME, user=None, **kwargs
    ):
        kwargs["project"] = project
        kwargs["name"] = name
        if not user:
    ...
    def _create_location_format(
        self, location_set_name, char_definition, user=None, **kwargs
    ):
    ...
    def _create_location_code(
        self, location_format, segment, parent=None, user=None, **kwargs
    ):
        """
        The kwargs can pass an updated segment.
        """
        try:
            obj = LocationCode.objects.get(
                location_format=location_format, parent=parent, segment=segment
            )
    ...
    def _create_subdivision(
        self, country, subdivision_name="New York", code="US-NY"
    ):
        kwargs = {
    ...
    def _create_language(self, country, code):
        kwargs = {"code": code, "country": country}
        obj = Language(**kwargs)
        obj.save()
        return obj

    def _create_timezone(self, country, zone, coordinates):
        kwargs = {"zone": zone, "coordinates": coordinates, "country": country}
        obj = TimeZone(**kwargs)
        obj.save()
        return obj
    ...
    def _create_item(
        self, project, column_collection, item_number, user=None, **kwargs
    ):
    ...
    def _create_invoice(
        self, project, currency, supplier, invoice_number, user=None, **kwargs
    ):
        kwargs["project"] = project
        kwargs["currency"] = currency
    ...
    def _create_invoice_item(
        self, invoice, item_number, quantity, unit_price, **kwargs
    ):
